...
const sharedStack = new SharedStack(app, "SharedStack", {
  description:
    "Creates shared S3 bucket and Lambda function to manage notifications on the bucket (qs-1s4376pnc)",
});
...
new AStack(app, "AStack", {
  description:
    "Stack that synthesizes S3 event notifications to an SQS queue (qs-1s4376pnc)",
  bucketName: sharedStack.bucketName,
});
new BStack(app, "BStack", {
  description:
    "Stack that synthesizes S3 event notifications to an SNS topic (qs-1s4376pnc)",
  bucketName: sharedStack.bucketName,
});
