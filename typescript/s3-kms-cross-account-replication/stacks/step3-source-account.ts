...
export class Step3SourceAccount extends Stack {
  constructor(scope: Construct, id: string, props?: StackProps) {
    ...
    const destinationKmsKeyArn = new CfnParameter(this, 'destinationKmsKeyArn', {
      type: 'String',
      description: 'The ARN of the destination KMS key in the destination account',
    });

    const destinationS3BucketArn = new CfnParameter(this, 'destinationS3BucketArn', {
      type: 'String',
    ...
    new CfnOutput(this, 'sourceS3BucketArn', { value: sourceS3Bucket.bucketArn });

    // no high level construct for replication for S3 yet, use low level contruct for now
    const lowLevelSourceS3Bucket = sourceS3Bucket.node.defaultChild as CfnBucket;
