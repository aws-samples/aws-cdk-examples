...
class ModelHandler(object):
    ...
    def get_model_files_prefix(self, model_dir):
        ...
        checkpoint_prefix = os.path.basename(checkpoint_prefix_filename).split(
            sym_file_suffix
        )[
            0
        ]  # Ex output: resnet18
    ...
    def get_input_data_shapes(self, model_dir, checkpoint_prefix):
        ...
        shapes_file_path = os.path.join(
            model_dir, "{}-{}".format(checkpoint_prefix, "shapes.json")
        )
    ...
    def postprocess(self, inference_output):
        ...
        a = np.argsort(prob)[::-1]
        return [
            ["probability=%f, class=%s" % (prob[i], self.labels[i]) for i in a[0:5]]
        ]
