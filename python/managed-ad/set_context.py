...
def create_secret_if_not_exists(secret_name):
    ...
    while True:
        try:
            ...
            use_existing = input(f"Secret '{secret_name}' already exists, do you want to use the existing secret? (Y/n) ").lower()
            if use_existing == 'n':
...
def get_ad_edition():
    ...
    ad_edition = input("Enter the Active Directory (AD) edition (default is Standard): ").upper().strip()
    
    if not ad_edition:
        return "Standard"
    
    if ad_edition in valid_editions:
        return ' '.join(''.join([w[0].upper(), w[1:].lower()]) for w in ad_edition.split())
    else:
...
def update_context(context_file, context):
    ...
    if update_context_file == 'y':
        ...
        context['vpc_id'] = input(f"Enter the VPC ID (default: {context['vpc_id']}): ") or context['vpc_id']
        context['internet_access'] = input(f"Enable internet access for the VPC [True/False] (default: {context['internet_access']}): ").title() or bool(context['internet_access'])
        context['ad_domain_name'] = input(f"Enter the AD domain name (default: {context['ad_domain_name']}): ") or context['ad_domain_name']
        context['ad_domain_edition'] = input(f"Enter the AD edition (default: {context['ad_edition']}): ") or context['ad_edition']
        context['secret_arn'] = input(f"Enter the AD password secret ARN (default: {context['secret_arn']}): ") or context['secret_arn']
